...
function SelectedTD(props: any) {
  ...
  useEffect(() => {
    SharePointService.getOperations(
      `/_api/web/lists/GetById('224c5b7d-d58f-45f0-a6ea-5c19c68f3914')/items?$select=ServerRedirectedEmbedUri,FileLeafRef,AccountNumber,DocType/Title,ItemStatus,Processor/Title,Modified,Editor/Title&$expand=Editor,Processor,DocType&$filter=RelationshipId eq '${props.relationshipId}'`
    ).then((res) => {
      setItems(res.value);
      setLoading(false);
    });
