...
function SelectedSignatureGuarantee(props:any) {
    const [loading, setLoading] = useState(true);
    const [items, setItems] = useState(null);
    const [filePickerResult, setfilePickerResult] = useState(null);

    useEffect(() => {    
        SharePointService.getOperations(`/_api/web/lists/GetById('f9c10533-42bb-46e8-b34d-c0aa7070b024')/items?$select=ServerRedirectedEmbedUri,FileLeafRef,AccountNumber,DateReceived,Recommendations,ReadyforReview,Reason,Amount,Product,Shares,RepID/Title,Processor/Title,Modified,Editor/Title&$filter=RelationshipId eq '${props.relationshipId}'&$expand=Editor,Processor,RepID`).then(
            (res) => {
                setItems(res.value);
                setLoading(false);
            }
...
export default SelectedSignatureGuarantee;