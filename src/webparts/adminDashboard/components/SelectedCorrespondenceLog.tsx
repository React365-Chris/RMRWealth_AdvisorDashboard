...
function SelectedCorrespondenceLog(props:any) {
    const [loading, setLoading] = useState(true);
    const [items, setItems] = useState(null);
    const [filePickerResult, setfilePickerResult] = useState(null);

    useEffect(() => {    
        SharePointService.getOperations(`/_api/web/lists/GetById('5011a439-fc91-4da5-9d98-8bcc317c43db')/items?$select=ServerRedirectedEmbedUri,FileLeafRef,CorrespondenceType,DateReceivedSent,Recommendations,ReadyforReview,Processor/Title,Modified,Editor/Title&$filter=RelationshipId eq '${props.relationshipId}'&$expand=Editor,Processor`).then(
            (res) => {
                setItems(res.value);
                setLoading(false);
            }
...
export default SelectedCorrespondenceLog;