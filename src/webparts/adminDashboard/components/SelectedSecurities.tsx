...
function SelectedSecurities(props:any) {
    const [loading, setLoading] = useState(true);
    const [items, setItems] = useState(null);
    const [filePickerResult, setfilePickerResult] = useState(null);

    useEffect(() => {    
        SharePointService.getOperations(`/_api/web/lists/GetById('462abf24-43b5-4d76-81da-9abc77a19904')/items?$select=ServerRedirectedEmbedUri,FileLeafRef,AccountNumber,DateReceived,ReadyforReview,Processor/Title,Modified,Editor/Title&$filter=RelationshipId eq '${props.relationshipId}'&$expand=Editor,Processor`).then(
            (res) => {
                setItems(res.value);
                setLoading(false);
            }
...
export default SelectedSecurities;